...
def _toolchains_toolchain_impl(ctx):
    ...
    template_variables = platform_common.TemplateVariableInfo({
        "toolchains_BIN": target_tool_path,
    })
    default = DefaultInfo(
        files = depset(tool_files),
        runfiles = ctx.runfiles(files = tool_files),
    )
    toolchains_info = ToolchainsInfo(
    ...
    toolchain_info = platform_common.ToolchainInfo(
        ToolchainsInfo = toolchains_info,
...
toolchains_toolchain = rule(
    implementation = _toolchains_toolchain_impl,
    attrs = {
    ...
    doc = """Defines a toolchains compiler/runtime toolchain.
